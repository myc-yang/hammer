...
static void test_read_bits_48(void) {
    {
        HInputStream is =
            MK_INPUT_STREAM("\x12\x34\x56\x78\x9A\xBC", 6, BIT_LITTLE_ENDIAN | BYTE_LITTLE_ENDIAN);
...
static void test_bug60() {
    ...
    plural_zed = h_sequence(why, h_many(h_choice(alpha, vchar, NULL)), NULL);
    plural_zed_zed = h_choice(plural_zed, zed, NULL);
    alphas = h_choice(alpha, h_sequence(plural_zed_zed, alpha, NULL), NULL);

    a_to_zed = h_sequence(zed, h_many(h_sequence(h_many1(alphas), zed, NULL)), NULL);
    rule = h_sequence(a_to_zed, plural_zed_zed, NULL);
    rulelist = h_many1(h_choice(rule, h_sequence(h_many(alphas), plural_zed_zed, NULL), NULL));
...
static void test_bug60_abnf() {
    ...
    equal = h_ch('=');
    semicolon = h_ch(';');
    comment = h_sequence(semicolon, h_many(h_choice(sp, vchar, NULL)), newline, NULL);
    ...
    alphas = h_sequence(alpha, h_many(h_sequence(h_many1(c_sp), alpha, NULL)), h_many(c_sp), NULL);
    rule = h_sequence(alpha, defined_as, alphas, c_nl, NULL);
    rulelist = h_many1(h_choice(rule, h_sequence(h_many(c_sp), c_nl, NULL), NULL));

...
void *h_arena_realloc(HArena *arena, void *ptr, size_t n) {
    ...
    for (link = arena->head; link; link = link->next) {
        if (ptr >= (void *)link->rest && ptr <= (void *)((char *)link->rest + link->used))  /* TODO for cFS: "error: pointer of type ‘void *’ used in arithmetic [-Werror=pointer-arith]" */
            break; /* found it */
    }
    assert(link != NULL);

    ncopy = ((char *)link->rest + link->used) - (char *)ptr;  /* TODO for cFS: "error: pointer of type ‘void *’ used in arithmetic [-Werror=pointer-arith]" */
    if (n < ncopy)
        ncopy = n;

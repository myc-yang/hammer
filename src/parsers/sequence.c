...
static HParseResult *parse_sequence(void *env, HParseState *state) {
    ...
    tok->swig_union.seq = seq;     /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    tok->index = 0;
    tok->bit_offset = 0;
    tok->bit_length = 0;
...
static HParsedToken *reshape_sequence(const HParseResult *p, void *user_data) {
    ...
    for (size_t i = 0; i < p->ast->swig_union.seq->used; i++) {    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        if (p->ast->swig_union.seq->elements[i] != NULL)            /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            h_carray_append(seq, p->ast->swig_union.seq->elements[i]); /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    }

...
succ:; // necessary for the label to be here...
    ...
    res->swig_union.seq = seq;     /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    res->index = 0;
    res->bit_length = 0;
    res->bit_offset = 0;
...
static HParsedToken *reshape_many(const HParseResult *p, void *user) {
    ...
    while (tok) {
        ...
        if (tok->swig_union.seq->used > 0) {   /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            size_t n = tok->swig_union.seq->used;      /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            assert(n <= 3);
            h_carray_append(seq, tok->swig_union.seq->elements[n - 2]);        /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            tok = tok->swig_union.seq->elements[n - 1];        /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        } else {
            tok = NULL;
        }
    }
...
static HParseResult *parse_length_value(void *env, HParseState *state) {
    ...
    HRepeat repeat = {.p = lv->value, .sep = NULL, .count = len->ast->swig_union.uint, .min_p = false};    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */

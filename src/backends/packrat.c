...
static HParserCacheValue *cached_result(HParseState *state, HParseResult *result) {
    ...
    ret->cache_value_union.right = result;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
...
static HParserCacheValue *cached_lr(HParseState *state, HLeftRec *lr) {
    ...
    ret->cache_value_union.left = lr;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ret->input_stream = state->input_stream;
    return ret;
}
    ...
    if (!head) {
        ...
        if (h_slist_find(head->eval_set, k->parser)) {
            ...
            if (!cached) {
                ...
                cached->cache_value_union.right = tmp_res;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
...
HParseResult *grow(HParserCacheKey *k, HParseState *state, HRecursionHead *head) {
    ...
    HParseResult *old_res = old_cached->cache_value_union.right;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    if (tmp_res) {
        if (pos_lt(old_cached->input_stream, state->input_stream)) {
            ...
            if (cached && PC_RIGHT == cached->value_type) {
                ...
                return cached->cache_value_union.right;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            } else {
                h_platform_errx(1, "impossible match");
            }
        }
...
HParseResult *h_do_parse(const HParser *parser, HParseState *state) {
    ...
    if (!m) {
        ...
        if (PC_LEFT == m->value_type) {
            setupLR(parser, state, m->cache_value_union.left);  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
            return m->cache_value_union.left->seed;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        } else {
            return m->cache_value_union.right;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        }
    }
}
...
bool h_packrat_parse_chunk(HSuspendedParser *s, HInputStream *input) {
    ...
    memcpy((void *)((char *)cat->input + cat->length), input->input, input->length);    /* TODO for cFS - "error: pointer of type ‘void *’ used in arithmetic [-Werror=pointer-arith]" */
    cat->length = newlen;

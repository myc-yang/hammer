...
HBenchmarkResults *h_benchmark__m(HAllocator *mm__, HParser *parser, HParserTestcase *testcases) {
    ...
    for (backend = PB_MIN; backend <= PB_MAX; backend++) {
        ...
        for (tc = testcases; tc->input != NULL; tc++) {
            ...
            int64_t time_diff;
            do {
                count *= 2; // Yes, this means that the first run will run the function twice. This
                            // is fine, as we want multiple runs anyway.
            ...
            ret->results[backend].cases[cur_case].swig_union_2.parse_time = (time_diff / count);    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    for (size_t i = 0; i < result->len; ++i) {
        ...
        for (size_t j = 0; j < result->results[i].n_testcases; ++j) {
            ...
            fprintf(stream, "Case %zd: %zd ns/parse, %zd ns/byte\n", j,
                    result->results[i].cases[j].swig_union_2.parse_time,                                            /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
                    result->results[i].cases[j].swig_union_2.parse_time / result->results[i].cases[j].length);      /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        }
    }
}

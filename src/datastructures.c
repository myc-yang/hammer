...
HCountedArray *h_carray_new(HArena *arena) { return h_carray_new_sized(arena, 4); }
...
void h_carray_append(HCountedArray *array, void *item) {
    ...
    if (array->used >= array->capacity) {
        ...
        elements = h_arena_malloc_noinit(array->arena, (array->capacity *= 2) * sizeof(void *));
...
HHashTable *h_hashtable_new(HArena *arena, HEqualFunc equalFunc, HHashFunc hashFunc) {
    ...
    ht->contents = h_arena_malloc_noinit(arena, sizeof(HHashTableEntry) * ht->capacity);
...
void *h_hashtable_get_precomp(const HHashTable *ht, const void *key, HHashValue hashval) {
    ...
    for (hte = &ht->contents[hashval & (ht->capacity - 1)]; hte != NULL; hte = hte->next) {
        if (hte->key == NULL) {
            continue;
        }
...
void h_hashtable_ensure_capacity(HHashTable *ht, size_t n) {
    ...
    if (do_resize) {
        ...
        new_contents = h_arena_malloc_noinit(ht->arena, sizeof(HHashTableEntry) * ht->capacity);
        ht->contents = new_contents;
        ht->used = 0;
        memset(new_contents, 0, sizeof(HHashTableEntry) * ht->capacity);
        for (size_t i = 0; i < old_capacity; ++i) {
            for (HHashTableEntry *entry = &old_contents[i]; entry; entry = entry->next) {
                if (entry->key) {
...
void h_hashtable_put_precomp(HHashTable *ht, const void *key, void *value, HHashValue hashval) {
    HHashTableEntry entry = {.key = key, .value = value, .hashval = hashval};
...
int h_hashtable_present(const HHashTable *ht, const void *key) {
    ...
    for (HHashTableEntry *hte = &ht->contents[hashval & (ht->capacity - 1)]; hte != NULL;
         hte = hte->next) {
        if (hte->hashval != hashval)
            continue;
        if (ht->equalFunc(key, hte->key))
...
static bool hte_same_length(HHashTableEntry *xs, HHashTableEntry *ys) {
    while (xs && ys) {
        ...
        while (xs && xs->key == NULL)
            xs = xs->next;
        while (ys && ys->key == NULL)
            ys = ys->next;
    }
...
static bool hte_subset(HEqualFunc eq, HEqualFunc value_eq, HHashTableEntry *xs,
                       HHashTableEntry *ys) {
    for (; xs; xs = xs->next) {
        if (xs->key == NULL)
            continue; // element not present

        HHashTableEntry *hte;
        for (hte = ys; hte; hte = hte->next) {
            // assume an element is equal to itself
            if (hte->key == xs->key && hte->value == xs->value)
                break;
            if (hte->hashval != xs->hashval)
                continue; // shortcut
            if (eq(hte->key, xs->key) && value_eq(hte->value, xs->value))
                break;
        }
        if (hte == NULL)
...
static inline bool hte_equal(HEqualFunc eq, HEqualFunc value_eq, HHashTableEntry *xs,
                             HHashTableEntry *ys) {
...
static bool eq_dontcare(const void *p, const void *q) { return true; }

...
HCFGrammar *h_cfgrammar_(HAllocator *mm__, HCFChoice *desugared) {
    ...
    if (h_hashset_empty(g->nts)) {
        ...
        nt->type = HCF_CHOICE;
        nt->hcfchoice_union.seq = h_new(HCFSequence *, 2);  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->hcfchoice_union.seq[0] = h_new(HCFSequence, 1); /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->hcfchoice_union.seq[0]->items = h_new(HCFChoice *, 2);  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->hcfchoice_union.seq[0]->items[0] = desugared;   /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->hcfchoice_union.seq[0]->items[1] = NULL;    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->hcfchoice_union.seq[1] = NULL;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        nt->pred = NULL;
        nt->action = NULL;
...
static void collect_nts(HCFGrammar *grammar, HCFChoice *symbol) {
    ...
    case HCF_CHOICE:
        ...
        for (s = symbol->hcfchoice_union.seq; *s != NULL; s++) {    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    do {
        ...
        for (i = 0; i < g->nts->capacity; i++) {
            for (hte = &g->nts->contents[i]; hte; hte = hte->next) {
                ...
                HCFSequence **p;
                for (p = symbol->hcfchoice_union.seq; *p != NULL; p++) {    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
...
static void remove_productions_with(HCFGrammar *g, const HCFChoice *x) {
    ...
    for (i = 0; i < g->nts->capacity; i++) {
        for (hte = &g->nts->contents[i]; hte; hte = hte->next) {
            ...
            HCFSequence **p, **q;
            for (p = symbol->hcfchoice_union.seq; *p != NULL;) {    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    do {
        ...
        for (i = 0; !found && i < g->nts->capacity; i++) {
            for (hte = &g->nts->contents[i]; !found && hte; hte = hte->next) {
                ...
                if (*symbol->hcfchoice_union.seq == NULL)   /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
                    found = true;
            }
        }
        if (found) {
...
static const HStringMap *h_first_work(size_t k, HCFGrammar *g, HHashTable **pws,
    ...
    switch (x->type) {
    case HCF_CHAR:
        h_stringmap_put_char(ret, x->hcfchoice_union.chr, INSET);   /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        break;
    case HCF_CHARSET:
        c = 0;
        do {
            if (charset_isset(x->hcfchoice_union.charset, c)) {     /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    case HCF_CHOICE:
        ...
        *pkx = kx;
        ...
        for (p = x->hcfchoice_union.seq; *p; ++p) {     /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    ...
    *pkx = kx;
    ...
    for (i = 0; i < g->nts->capacity; i++) {
        for (hte = &g->nts->contents[i]; hte; hte = hte->next) {
            ...
            HCFSequence **p;
            for (p = a->hcfchoice_union.seq; *p; p++) {     /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
...
static HCFChoice **pprint_string(FILE *f, HCFChoice **x) {
    ...
    for (; *x; x++) {
        ...
        h_pprint_char(f, (*x)->hcfchoice_union.chr);    /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    }
    fputc('"', f);
    return x;
}
...
void h_pprint_symbol(FILE *f, const HCFGrammar *g, const HCFChoice *x) {
    ...
    case HCF_END:
        ...
        break;
    case HCF_CHARSET:
        pprint_charset(f, x->hcfchoice_union.charset);  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
        break;
    default:
...
static void pprint_ntrules(FILE *f, const HCFGrammar *g, const HCFChoice *nt, int indent, int len) {
    ...
    HCFSequence **p = nt->hcfchoice_union.seq;  /* TODO for cFS - Added union name - ISO C99 doesn’t support unnamed structs/unions */
    if (*p == NULL) {
